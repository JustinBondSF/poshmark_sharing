...
def login(debugger=False):
    ...
    try:
        ...
        # Captcha Catch
        print(textwrap.dedent('''
            [*] ERROR in Sharing: Thrwarted by Captchas
...
def get_closet_share_icons():
    ...
    if not share_icons:
        ...
        items = driver.find_elements_by_xpath(item_pat)
        share_icons = [i.find_element_by_css_selector(
            ".share-gray-large") for i in items]
    return share_icons


def clicks_share_followers(share_icon, d=3, d2=1):
...
def deploy_share_war(n=10, order=True, random_subset=0):
    print("[*] Lets get started! Using the login credentials provided in \"credentials.py\"")
    ...
    try:
        ...
        items_in_closet = len(share_icons)


        print(textwrap.dedent('''
            [*]  Items for sale in your closet: {}

            Please wait while they are shared.



            ''' ).format(items_in_closet))

        # Share Loops
        def ProgressBar(items_in_closet):



            percent_complete = int((index+1)/(items_in_closet)*100)
            if (percent_complete < 100):
                if percent_complete // 2 == 0 :
                    bar = "[..................................................]"
                    marker = ">"
                    bar.replace(".", marker,int(percent_complete/2))
                    return bar
        ...
        for item in share_icons:
            ...
            index = share_icons.index(item)

            elapsedTimer = timer()
            elapsedTime = time.strftime("%H:%M:%S", time.gmtime(elapsedTimer))
            rough_eta = (len(share_icons) * 7)
            eta = time.strftime("%H:%M:%S", time.gmtime(rough_eta))

            eta = float(rough_eta- elapsedTimer)

            eta = time.strftime("%H:%M:%S", time.gmtime(eta))


            ProgressBar(items_in_closet)


            print(textwrap.dedent(f'''
            [*] shared: {index} of {items_in_closet} [*]
            [*] ETA: {eta}         [*]
        ...
        print(
            "[*] Successfully shared {} items in your closet!".format(items_in_closet))
        pass
        print("[*] ERROR in Sharing your listings!")
        pass
    ...
    print(textwrap.dedent('''
        [*] Your listings will be shared again in {} minutes...
            current time: {}
